...
        card.innerHTML = `
  _         <img src="${item.imageUrl}" alt="${item.title}" class="w-full h-48 object-cover">
...
    // --- Data Rendering ---
    // FIX: Check if data exists before trying to render it.
    // This prevents errors on pages that don't have this data (e.g., news.html)
    
    // The 'publications' variable is checked inside the function, so this call is safe.
    renderPublications(4); 
    
    // These calls will crash if the variables are not defined.
    // We add a check to see if the variable exists before calling the function.
    if (typeof teamMembers !== 'undefined') {
        renderTeam(teamMembers, 'team-members-list'); 
    }
    if (typeof postdocAlumni !== 'undefined') {
        renderTeam(postdocAlumni, 'team-alumni-postdoc-list'); 
    }
    if (typeof mscAlumni !== 'undefined') {
        renderTeam(mscAlumni, 'team-alumni-msc-list'); 
    }
    if (typeof internAlumni !== 'undefined') {
        renderTeam(internAlumni, 'team-alumni-intern-list');
    }
    
    // These functions use data defined in this file (newsItems, galleryImages)
    // and are safe to run. renderNews() will now be executed.
    renderNews();
    renderGallery();
...
                if (window.scrollY >= sectionTop - 90) {s
...
nbsp;       modalExcerpt.innerHTML = excerpt.split('\n').map(p => `<p>${p}</p>`).join('');
...
s       setTimeout(() => newsModal.classList.add('hidden'), 300); // Hide after fade-out
    };
